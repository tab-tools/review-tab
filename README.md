...
- Switch between two review scopes with completely independent selections: **Last Prompt** shows only what Claude changed in the most recent turn; **All Changes** shows everything uncommitted in the working tree vs the last git commit, across all turns. Each mode keeps its own keep/restore/partial state so you can review the same files differently in each scope. Your selections are always saved and can be revisited at any time.
...
- Apply the review once you're satisfied. The apply modal confirms which **mode's selections** will be applied (Last Prompt or All Changes), so you never accidentally apply selections from the wrong scope. Only the current mode's selections are applied; the other mode's selections are preserved and untouched
...
- The browser opens on its own once a review is ready, and does so instantly for the first review in a given project. If a review is already open and Claude makes new changes, the existing tab refreshes in place with the updated diff — no new tab opens. This keeps your focus in one place. When you close a tab, it sends a close beacon so the server knows, ensuring the next review can open a fresh tab even if the diff hasn't changed.
- Mode selections are stored independently per mode under the key `cr-state-{mode}-{sig}` in both sessionStorage and localStorage. This means you can switch between Last Prompt and All Changes modes, make different keep/restore decisions in each, and both will persist. The apply modal displays which mode you're currently in, so you never accidentally apply the wrong mode's selections. Only the active mode's selections are applied; the other mode's selections remain untouched and can be revisited later.

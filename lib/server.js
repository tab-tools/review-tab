...
function serve(repoRoot) {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === "GET" && req.url === "/api/status") {
      res.writeHead(200, { "Content-Type": "application/json" });
      // 15 sec tolerance: closing tab triggers /api/close (lastPoll=0 instantly); 15s window covers throttled background tabs.
      res.end(JSON.stringify({
        viewing: lastPoll > 0 && Date.now() - lastPoll < 15000,
        version: VERSION,

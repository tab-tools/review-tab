...
async function openReview(repoRoot, { manual, hookInput } = {}) {
  ...
  // Don't reopen for the same diff — Stop hook fires every turn.
  ...
  // Don't spawn a duplicate; always replace a server from an older plugin version.
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === "GET" && req.url === "/api/sig") {
      ...
      res.end(JSON.stringify({
        ...
        updateAvailable: us ? us.updateAvailable : false,
    ...
    if (req.method === "GET" && req.url === "/api/status") {
      res.writeHead(200, { "Content-Type": "application/json" });
      // 2 min tolerance: browsers throttle background-tab timers so polls can lag significantly.
      res.end(JSON.stringify({
    ...
    if (req.method === "POST" && req.url === "/api/update") {
      ...
      exec(
        ...
        (err, _stdout, stderr) => {
          ...
          log(repoRoot, `update to v${latestVersion} succeeded`);
          res.writeHead(200, { "Content-Type": "application/json" });
